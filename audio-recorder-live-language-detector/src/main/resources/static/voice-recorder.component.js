...
class VoiceRecorderComponent {
  constructor(root) {
    ...
    this.targetLangValue = 'en';
    this.fileValue = null;
    this.recordingModeValue = 'audio';
  ...
  bindEvents() {
    ...
    fileInput && fileInput.addEventListener('change', (event) => {
      ...
      this.fileValue = file || null;
    });

    submitBtn && submitBtn.addEventListener('click', () => this.uploadFile());
    startBtn && startBtn.addEventListener('click', () => this.startRecording());
    stopBtn && stopBtn.addEventListener('click', () => this.stopRecording());

    langSelect && langSelect.addEventListener('change', (event) => {
      this.targetLangValue = event.target.value;
    });

    modeSelect && modeSelect.addEventListener('change', (event) => {
      this.recordingModeValue = event.target.value || 'audio';
    });
  }
  ...
  startRecording() {
    ...
    const constraints = this.recordingModeValue === 'video'
  ...
  stopRecording() {
    ...
    this.mediaRecorder.onstop = () => {
      const mimeType = this.mediaRecorder && this.mediaRecorder.mimeType
        ...
        : (this.recordingModeValue === 'video' ? 'video/webm' : 'audio/webm');
